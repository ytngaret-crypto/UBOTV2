...
class SettingsStore:
    ...
    async def set(self, chat_id, feature, setting, value):
        def work():
            with self._conn() as c:
                ...
                ON CONFLICT(chat_id,feature,setting) DO UPDATE SET value=excluded.value""",
    ...
    async def get(self, chat_id, feature, setting, default=None):
        def work():
            with self._conn() as c:
                return c.execute("SELECT value FROM feature_settings WHERE chat_id=? AND feature=? AND setting=?", (chat_id,feature,setting)).fetchone()
    ...
    async def all(self, chat_id, feature):
        def work():
            return sqlite3.connect(self.db_path).execute("SELECT setting,value FROM feature_settings WHERE chat_id=? AND feature=?", (chat_id,feature)).fetchall()
